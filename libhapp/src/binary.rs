...
use crate::memory::{ceil, is_aligned, round_up};
...
impl Binary {
    ...
    fn check(elf: &Elf, size: usize) -> bool {
        ...
        let length =
            (header.program_header_entry_num() * header.program_header_entry_size()) as u64;
        ...
        let length =
            (header.section_header_entry_num() * header.section_header_entry_size()) as u64;
        let end = offset + length;
        if (size as u64) < end || end < offset {
            return false;
        }
