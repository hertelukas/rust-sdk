...
use crate::memory::{ceil, round_down, round_up};
...
impl Builder {
    ...
    pub fn setup_shared_memory(&mut self, base: uintptr, size: usize) -> Result<(), Error> {
        if base == 0 || size == 0 {
    ...
    pub fn build(&self, device: Option<&Device>) -> Result<Output, Error> {
        ...
        // Create memory
        let mut memory = if let Some(dev) = device {
        ...
        let params_as_bytes: &[u8] = unsafe {
            core::slice::from_raw_parts(
                (&params as *const RuntimeParams) as *const u8,
        ...
        */
        ...
        let shrd_base = if let Some(device) = device {
            device.finalize_enclave(ert_start, app_start, free_start, &params)?;
